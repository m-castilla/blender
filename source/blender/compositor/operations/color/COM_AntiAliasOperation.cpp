...
void AntiAliasOperation::execPixels(ExecutionManager &man)
{
  ...
  auto cpu_write = [&](PixelsRect &dst, const WriteRectContext &ctx) {
    ...
    CPU_READ_OFFSET(input, dst);
    int x = dst_start_x + write_offset_x;
    int y = dst_start_y + write_offset_y;

    if (y < 0 || y >= input_h || x < 0 || x >= input_w) {
      dst_img.buffer[dst_offset] = 0.0f;
    }
    else {
      const float *row_curr = &input_img.buffer[y * input_w];
      if (x == 0 || x == input_w - 1 || y == 0 || y == input_h - 1) {
        dst_img.buffer[dst_offset] = row_curr[x];
      }
      else {
        const float *row_prev = &input_img.buffer[(y - 1) * input_w],
                    *row_next = &input_img.buffer[(y + 1) * input_w];
        float ninepix[9];
