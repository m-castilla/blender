...
void DoubleEdgeMaskOperation::hashParams()
{
  NodeOperation::hashParams();
  hashParam(m_adjecentOnly);
  hashParam(m_keepInside);
}

void DoubleEdgeMaskOperation::execPixels(ExecutionManager &man)
{
  auto inner_mask = getInputOperation(0)->getPixels(this, man);
  auto outer_mask = getInputOperation(1)->getPixels(this, man);
  auto cpuWrite = [&](PixelsRect &dst, const WriteRectContext &ctx) {
    BLI_assert(ctx.n_rects == 1);
    UNUSED_VARS(ctx);

    int w = getWidth();
    int h = getHeight();
    BufferRecycler *recycler = GlobalMan->BufferMan->recycler();
    TmpBuffer *edge_tmp_buf = recycler->createTmpBuffer(true);
    recycler->takeNonStdRecycle(edge_tmp_buf, w, h, 1);

    auto inner_mask_dup = inner_mask->duplicate();
    auto outer_mask_dup = outer_mask->duplicate();

    float *edge_buf = edge_tmp_buf->host.buffer;
    doDoubleEdgeMask(
        inner_mask_dup.tmp_buffer->host.buffer, outer_mask_dup.tmp_buffer->host.buffer, edge_buf);

    WRITE_DECL(dst);

    CPU_LOOP_START(dst);

    int index = dst_coords.y * w + dst_coords.x;
    dst_img.buffer[dst_offset] = edge_buf[index];

    CPU_LOOP_END;

    recycler->giveRecycle(edge_tmp_buf);
    recycler->giveRecycle(inner_mask_dup.tmp_buffer);
    recycler->giveRecycle(outer_mask_dup.tmp_buffer);
  };
