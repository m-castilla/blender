...
void EllipseMaskOperation::hashParams()
{
...
ccl_kernel ellipseMaskOp(CCL_WRITE(dst),
                         CCL_READ(mask),
                         CCL_READ(value),
                         int mask_type,
                         ...
                         const float sine)
{
  ...
  float sx = rx - mask_middle_x;
  sx *= sx;
  float sy = ry - mask_middle_y;
  sy *= sy;
...
void EllipseMaskOperation::execPixels(ExecutionManager &man)
{
  ...
  const float tx = mask_half_w * mask_half_w;
  const float ty = mask_half_h * mask_half_h;

  auto cpu_write = std::bind(CCL::ellipseMaskOp,
                             _1,
                             mask,
                             value,
                             m_maskType,
                             ...
                             cosine,
                             sine);
  computeWriteSeek(man, cpu_write, "ellipseMaskOp", [&](ComputeKernel *kernel) {
    ...
    kernel->addFloatArg(tx);
    kernel->addFloatArg(ty);
