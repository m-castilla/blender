                                          ...
                                          const CompositorContext &context) const
{
  ...
  if (inputSocket->isLinked()) {
    ...
    float scale_new_w = rd->xsch * rd->size / 100.0f;
    float scale_new_h = rd->ysch * rd->size / 100.0f;
    ScaleFixedSizeOperation *scaleOperation = new ScaleFixedSizeOperation(scale_new_w,
                                                                          scale_new_h);
    scaleOperation->setIsAspect(false);
    scaleOperation->setIsCrop(false);
    scaleOperation->setOffset(0.0f, 0.0f);
    scaleOperation->getInputSocket(0)->setResizeMode(InputResizeMode::NO_RESIZE);
