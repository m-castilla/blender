...
static std::pair<std::string, std::string> loadKernelsSource()
{
...
blender::Span<ComputeDevice *> OpenCLManager::getDevices()
{
  return blender::Span((ComputeDevice **)m_devices.data(), m_devices.size());
}
...
OpenCLManager::~OpenCLManager()
{
  if (isInitialized()) {
    OpenCLDevice *device;
    while (!m_devices.is_empty()) {
      device = m_devices.pop_last();
      delete device;
    }

    OpenCLPlatform *platform;
    while (!m_platforms.is_empty()) {
      platform = m_platforms.pop_last();
      delete platform;
    }
  }
}
